-- ─────────────────────────────────────────────────────────────────────────────
-- Word Snake — Host-configurable target score
--
-- Adds an optional targetScore to the game state.  When set, the first player
-- whose score reaches that threshold wins immediately, regardless of how many
-- lives other players still have.  Null = elimination-only mode (original).
--
-- Changes:
--   word_snake_start  — now accepts p_target_score int (default null)
--   word_snake_submit — checks target-score win in addition to last-player win
--   word_snake_rematch — preserves targetScore across rematches
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  update public.game_session
     set state = jsonb_set(
           ...
           '{targetScore}', to_jsonb(p_target_score)
         ),
         status = 'active'
   where id = p_session_id;
end;
...
grant execute on function word_snake_start(uuid, int) to authenticated, anon;
...
begin
  ...
  if v_is_valid then
    ...
    -- Time-based scoring (turn started 10 000 ms before turnExpiresAt)
...
begin
  ...
  update public.game_session
     set state = jsonb_build_object(
           ...
           'targetScore',   v_sess.state -> 'targetScore'  -- preserve across rematches
         ),
         status = 'active'
   where id = p_session_id;
end;
